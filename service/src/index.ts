...
import { isMailConfigReady, sendNoticeMail, sendResetPasswordMail, sendTestMail, sendVerifyMail, sendVerifyMailAdmin } from './utils/mail'
...
router.post('/user-register', authLimiter, async (req, res) => {
  try {
    ...
    const rootUser = process.env.ROOT_USER?.trim().toLowerCase()
    const bootstrapAdmin = !rootUser && !await hasAdminUser()
    const isRoot = rootUser ? normalizedUsername === rootUser : bootstrapAdmin
    const canSendMail = isMailConfigReady(config.mailConfig)
    if (isNotEmptyString(config.siteConfig.registerMails)) {
      ...
      for (let index = 0; index < emailSuffixs.length; index++) {
        ...
        allowSuffix = normalizedUsername.endsWith(element)
        if (allowSuffix)
          break
      }
      if (!allowSuffix) {
    ...
    if (user != null) {
      if (user.status === Status.PreVerify) {
        if (!canSendMail)
          throw new Error('Mail service is not configured and the account is still pending activation.')
    ...
    const roles = isRoot ? [UserRole.Admin] : [UserRole.User]
    const status = (!isRoot && !canSendMail) ? Status.Normal : null
    const remark = bootstrapAdmin ? 'Bootstrap admin created from the first self-registration.' : undefined
    await createUser(username, newPassword, roles, status, remark)

    if (isRoot) {
      if (bootstrapAdmin) {
        res.send({ status: 'Success', message: 'The first registered account has been promoted to administrator.', data: null })
        return
      }
      ...
      if (!canSendMail) {
        res.send({ status: 'Success', message: 'Registration succeeded and the account was activated directly because mail is not configured.', data: null })
        return
      }
      await sendVerifyMail(username, await getUserVerifyUrl(username))
...
router.post('/user-send-reset-mail', authLimiter, async (req, res) => {
  try {
    ...
    const config = await getCacheConfig()
    if (!isMailConfigReady(config.mailConfig))
      throw new Error('Mail service is not configured, so reset email cannot be sent.')
...
router.post('/verify', authLimiter, async (req, res) => {
  try {
    ...
    if (config.siteConfig.registerReview) {
      if (!isMailConfigReady(config.mailConfig) || !isNotEmptyString(process.env.ROOT_USER)) {
        await verifyUser(username, Status.Normal)
        message = 'Verify successfully. The account was activated directly because mail or admin review is not configured.'
