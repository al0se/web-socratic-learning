...
export function isMailConfigReady(config?: MailConfig | null) {
  if (!config)
    return false

  return isNotEmptyString(config.smtpHost)
    && isNotEmptyString(config.smtpUserName)
    && isNotEmptyString(config.smtpPassword)
}
